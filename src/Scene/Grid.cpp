...
Grid::Grid(int size, int divisions) {
  ...
  // special status (not selectable, rendered statically) a property of the
  // object itself, rather than requiring special checks elsewhere in the code.
  isSelectable = false;
  isStatic = true;
...
std::string Grid::GetTypeString() const { return "Grid"; }
