...
UI::UI(Scene *scene)
    : m_Scene(scene), m_Factory(nullptr), m_RenameID(0), m_RenameBuffer() {}
...
void UI::EndFrame() { ImGui::Render(); }

void UI::SetObjectFactory(SceneObjectFactory *factory) { m_Factory = factory; }
...
void UI::DrawPropertiesPanel() {
  ...
  if (ImGui::CollapsingHeader("Properties", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    glm::decompose(selected->transform, scale, rotation, position, skew,
                   perspective);
    ...
    transformChanged |=
        ImGui::DragFloat3("Rotation", glm::value_ptr(euler), 1.0f);
      ...
      for (const auto &p : props) {
        switch (p.type) {
          case PropertyType::Float:
            propChanged |= ImGui::DragFloat(
                p.name.c_str(), static_cast<float *>(p.value_ptr), 0.05f);
            break;
          case PropertyType::Vec3:
            propChanged |= ImGui::DragFloat3(
                p.name.c_str(),
                glm::value_ptr(*static_cast<glm::vec3 *>(p.value_ptr)), 0.05f);
            break;
          case PropertyType::Color_Vec4:
            propChanged |= ImGui::ColorEdit4(
                p.name.c_str(),
                glm::value_ptr(*static_cast<glm::vec4 *>(p.value_ptr)));
            break;
        }
      }
      if (propChanged) {
