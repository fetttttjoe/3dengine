...
// We define the shaders here as string literals to avoid needing separate
// files.
...
void TransformGizmo::SetTarget(ISceneObject* target) {
  ...
  if (m_Target) {
    ...
    for (const auto& prop : props) {
      if (prop.name == "Width") {
        m_DimensionPointers[GizmoDimension::WIDTH] =
            static_cast<float*>(prop.value_ptr);
      }
      if (prop.name == "Height") {
        m_DimensionPointers[GizmoDimension::HEIGHT] =
            static_cast<float*>(prop.value_ptr);
      }
      if (prop.name == "Depth") {
        m_DimensionPointers[GizmoDimension::DEPTH] =
            static_cast<float*>(prop.value_ptr);
      }
    }
    CreateHandles();
...
void TransformGizmo::CreateHandles() {
  ...
  // This makes the gizmo correctly support objects that might not have all
  // three dimensions (like a 2D triangle).
  if (m_DimensionPointers.count(GizmoDimension::WIDTH)) {
    m_Handles.push_back({currentId++,
                         {1.0f, 0.0f, 0.0f},
                         {1, 0, 0, 1},
                         GizmoDimension::WIDTH,
                         1.0f});
    m_Handles.push_back({currentId++,
                         {-1.0f, 0.0f, 0.0f},
                         {1, 0, 0, 1},
  ...
  // A triangle has 0 depth, so a depth handle is not useful. We also check if
  // the pointer exists.
  if (m_DimensionPointers.count(GizmoDimension::DEPTH) &&
      *m_DimensionPointers[GizmoDimension::DEPTH] > 0.0f) {
    m_Handles.push_back({currentId++,
                         {0.0f, 0.0f, 1.0f},
                         {0, 0, 1, 1},
                         GizmoDimension::DEPTH,
                         1.0f});
    m_Handles.push_back({currentId++,
                         {0.0f, 0.0f, -1.0f},
                         {0, 0, 1, 1},
...
void TransformGizmo::Update(const Camera& camera, const glm::vec2& mouseDelta,
  ...
  glm::mat4 objectRotationMatrix =
      glm::mat4_cast(glm::quat(glm::radians(m_Target->GetEulerAngles())));
  glm::vec3 axisWorldDir = glm::normalize(
      glm::vec3(objectRotationMatrix * glm::vec4(handleAxisLocal, 0.0f)));
...
void TransformGizmo::Draw(const Camera& camera) {
  ...
  // Scale handles based on distance to camera to maintain a constant screen
  // size
  float distance = glm::length(camera.GetPosition() - m_Target->GetPosition());

  // FIX: Reduced the scaling factor to make the gizmo handles smaller and less
  // obtrusive.
  ...
  for (const auto& handle : m_Handles) {
    ...
    // Calculate handle world position based on object's transform and
    // dimensions
    float w = m_DimensionPointers.count(GizmoDimension::WIDTH)
                  ? *m_DimensionPointers.at(GizmoDimension::WIDTH) * 0.5f
                  : 0.f;
    float h = m_DimensionPointers.count(GizmoDimension::HEIGHT)
                  ? *m_DimensionPointers.at(GizmoDimension::HEIGHT) * 0.5f
                  : 0.f;
    float d = m_DimensionPointers.count(GizmoDimension::DEPTH)
                  ? *m_DimensionPointers.at(GizmoDimension::DEPTH) * 0.5f
                  : 0.f;

    glm::vec3 handlePosLocal = {handle.basePosition.x * w,
                                handle.basePosition.y * h,
                                handle.basePosition.z * d};

    glm::vec3 handleWorldPosition =
        glm::vec3(m_Target->transform * glm::vec4(handlePosLocal, 1.0f));
...
void TransformGizmo::DrawForPicking(const Camera& camera,
  ...
  // FIX: Reduced the scaling factor to match the visual size in the Draw()
  // method. This ensures the clickable area is consistent with what the user
  // sees.
  float scale = distance * 0.02f;
