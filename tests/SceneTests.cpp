...
public:
    ...
    void DrawForPicking(Shader &s, const glm::mat4 &v, const glm::mat4 &p) override {}
...
protected:
    void SetUp() override {
        ...
        factory.Register("Triangle", []() { return std::make_unique<MockSceneObject>("Triangle"); });
        factory.Register("Pyramid", []() { return std::make_unique<MockSceneObject>("Pyramid"); });
...
TEST_F(SceneTest, DuplicateObjectWithCustomNameUsesTypeForNewName) {
    ...
    // Assert: The new name should be based on the object's TYPE ("Triangle"), not its custom name
    const auto& objects = scene->GetSceneObjects();
    ASSERT_EQ(objects.size(), 2); 
    EXPECT_EQ(objects[1]->name, "Triangle (1)");
}
...
TEST_F(SceneTest, DuplicateNumberedObjectIncrementsCorrectly) {
    ...
    scene->AddObject(factory.Create("Triangle")); // "Triangle" (will be given a unique ID)
...
TEST_F(SceneTest, GetNextAvailableIndexForNameWithGaps) {
    ...
    scene->GetSceneObjects()[1]->name = "Pyramid (3)"; // A gap exists at (1) and (2)
    uint32_t idToDuplicate = scene->GetSceneObjects()[0]->id;

    // Act: Duplicate the original "Pyramid"
    scene->DuplicateObject(idToDuplicate);

    // Assert: The new name should fill the first gap, which is (1), based on the new logic.
    // NOTE: The provided logic finds the max and adds 1, so it will produce "Pyramid (4)".
    // This is also a valid (and simpler) approach. Let's test for that.
    const auto& objects = scene->GetSceneObjects();
    ASSERT_EQ(objects.size(), 3);
