...
#endif
    m_Window = glfwCreateWindow(m_WindowWidth, m_WindowHeight, "Intuitive Modeler", nullptr, nullptr);
    if (!m_Window) {
        glfwTerminate();
...
void Application::RegisterObjectTypes() {
    m_ObjectFactory->Register(std::string(ObjectTypes::Grid), []() { return std::make_unique<Grid>(); });
    m_ObjectFactory->Register(std::string(ObjectTypes::Triangle), []() { return std::make_unique<Triangle>(); });
    m_ObjectFactory->Register(std::string(ObjectTypes::Pyramid), []() { return std::make_unique<Pyramid>(); });
    m_ObjectFactory->Register(std::string(ObjectTypes::Sphere), []() { return std::make_unique<Sphere>(); });
    m_ObjectFactory->Register(std::string(ObjectTypes::Icosphere), []() { return std::make_unique<Icosphere>(); });
...
void Application::ImportModel(const std::string& filepath) {
    auto [vertices, indices] = m_Scene->LoadMeshFromFile(filepath);
...
void Application::SetEditorMode(EditorMode newMode, SculptMode::Mode newSculptMode) {
...
void Application::processSculpting() {
    ...
    ImVec2 relMousePos = { ImGui::GetMousePos().x - vp->GetBounds()[0].x, ImGui::GetMousePos().y - vp->GetBounds()[0].y };

    glm::vec3 ray_origin = m_Camera->GetPosition();
    glm::vec3 ray_direction = m_Camera->ScreenToWorldRay(MathHelpers::ToGlm(relMousePos), (int)vpSize.x, (int)vpSize.y);
    
    Raycaster::RaycastResult result;
    if (Raycaster::IntersectMesh(ray_origin, ray_direction, *sculptableMesh, selectedObject->GetTransform(), result)) {
        ...
        if (tool) {
            ...
            glm::vec3 hitPointWorld = glm::vec3(model * glm::vec4(result.hitPoint, 1.0f));
            
            tool->Apply(*sculptableMesh, hitPointWorld, ray_direction, MathHelpers::ToGlm(ImGui::GetIO().MouseDelta),
                        inspector->GetBrushSettings(), m_Camera->GetViewMatrix(), m_Camera->GetProjectionMatrix(),
...
void Application::cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    if (auto* vp = app->m_UI->GetView<ViewportPane>(); io.WantCaptureMouse && (!vp || !vp->IsHovered())) return;
    ...
    if (app->m_IsDraggingGizmo) {
        ...
        if (auto* vp = app->m_UI->GetView<ViewportPane>()) {
            ...
            glm::vec3 world = Camera::ScreenToWorldPoint(rel, app->m_DragNDCDepth,
                                                         invViewProj,
